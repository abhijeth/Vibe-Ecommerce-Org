 ...
 * 
 * This application demonstrates "insecure by default" design with maximum vulnerabilities:
 * - All Version 1 & 2 vulnerabilities PLUS:
 * - Complete authentication bypass
 * - Role bypass (any user can be admin/owner)
 * - Broken security features (MFA, rate limiting)
 * - Maximum PII exposure
 * - No input validation
 * - "Insecure by default" design philosophy
 * 
...
var (
	...
	// ============================================================================
// SECURITY VARIABLES (VULNERABLE IMPLEMENTATION)
// ============================================================================

// Security variables (implemented but bypassable)
...
// findAvailablePort finds an available port in the range 10001-11000
// ============================================================================
// UTILITY FUNCTIONS
// ============================================================================
...
func hashPassword(password string) string {
	// ⚠️ VULNERABILITY: bcrypt implemented but authentication can be bypassed
...
func isAccountLocked(email string) bool {
	...
	if lockoutTime[email].IsZero() {
		return false
	}

	if time.Since(lockoutTime[email]) < 15*time.Minute {
		return true
	}
...
func initDatabase() {
	// Create users table with PII and security fields
	_, err := db.Exec(`
...
func seedInitialData() {
	...
	for _, user := range users {
		...
		_, err := db.Exec(`
			INSERT INTO users (email, password, first_name, last_name, phone, ssn, date_of_birth, address, city, state, zip_code, credit_card_number, credit_card_expiry, credit_card_cvv, mfa_enabled, is_user, is_admin, is_owner)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		`, user.Email, hashedPassword, user.FirstName, user.LastName, user.Phone, user.SSN, user.DateOfBirth, user.Address, user.City, user.State, user.ZipCode, user.CreditCardNumber, user.CreditCardExpiry, user.CreditCardCVV, true, user.IsUser, user.IsAdmin, user.IsOwner)
		if err != nil {
 ...
 * Version 3: "Insecure by default" with maximum vulnerabilities
 */
func main() {
	r := mux.NewRouter()
	...
	fmt.Printf("🚀 Vibe E-commerce Version 3 starting on http://localhost:%d\n", port)
	fmt.Printf("⚠️  WARNING: This is an intentionally vulnerable application for educational purposes!\n")
	fmt.Printf("💥 Version 3: Insecure by Default - Maximum vulnerabilities with broken security features\n")
	fmt.Printf("🔓 Complete authentication bypass - any user can access admin/owner functions\n")
...
func loginHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method == "POST" {
		...
		var user User
		err := db.QueryRow(`
			SELECT id, email, password, mfa_enabled, mfa_secret, last_login_attempt, 
			       failed_login_attempts, account_locked_until, is_user, is_admin, is_owner 
			FROM users WHERE email = ?
		`, email).Scan(
			&user.ID, &user.Email, &user.Password, &user.MFAEnabled, &user.MFASecret,
			&user.LastLoginAttempt, &user.FailedLoginAttempts, &user.AccountLockedUntil,
			&user.IsUser, &user.IsAdmin, &user.IsOwner)
		...
		if !user.AccountLockedUntil.IsZero() && time.Now().Before(user.AccountLockedUntil) {
			http.Redirect(w, r, "/login?error=locked", http.StatusSeeOther)
			return
		}

		// Verify password (now hashed)
		err = bcrypt.CompareHashAndPassword([]byte(user.Password), []byte(password))
		if err != nil {
			// Update failed login attempts
			db.Exec("UPDATE users SET failed_login_attempts = failed_login_attempts + 1, last_login_attempt = ? WHERE email = ?",
				time.Now(), email)

			// Check if should lock account
			if user.FailedLoginAttempts >= 4 { // 5th attempt
				lockoutUntil := time.Now().Add(15 * time.Minute)
				db.Exec("UPDATE users SET account_locked_until = ? WHERE email = ?", lockoutUntil, email)
			}
		...
		// MFA disabled for now - proceed with login

		// Clear failed login attempts on successful login
		db.Exec("UPDATE users SET failed_login_attempts = 0, account_locked_until = NULL WHERE email = ?", email)
...
<body>
    ...
    <div class="container">
        ...
        <div class="test-credentials">
            <h3>Test Credentials (SECURED):</h3>
            <p><strong>Customer:</strong> alice@example.com / SecurePass123!</p>
            <p><strong>Admin:</strong> admin@example.com / AdminPass456!</p>
            <p><strong>Owner:</strong> owner@example.com / OwnerPass789!</p>
            <p><em>Note: All accounts have strong password policy and rate limiting enabled.</em></p>
        </div>
...
func registerHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method == "POST" {
		...
		isValid, errorMsg := validatePasswordStrength(password)
		if !isValid {
		...
		_, err := db.Exec("INSERT INTO users (email, password, is_user, mfa_enabled) VALUES (?, ?, 1, 1)",
...
func productsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if searchQuery != "" {
		// Use parameterized query to prevent SQL injection
		rows, err := db.Query("SELECT id, name, description, price, image_url FROM products WHERE LOWER(name) LIKE LOWER(?) OR LOWER(description) LIKE LOWER(?)",
...
<head>
    ...
    <style>
        ...
        .search-container { margin: 2rem 0; padding: 1rem; background: #f8f9fa; border-radius: 8px; }
        .search-form { display: flex; gap: 1rem; align-items: center; }
        .search-input { flex: 1; padding: 0.75rem; border: 1px solid #ddd; border-radius: 4px; font-size: 1rem; }
        .search-btn { background: #28a745; color: white; padding: 0.75rem 1.5rem; border: none; border-radius: 4px; cursor: pointer; font-size: 1rem; }
        .search-btn:hover { background: #218838; }
        .search-results { margin-top: 1rem; color: #666; font-style: italic; }
...
<body>
	...
	html += `<div class="search-container">
        <form method="GET" action="/products" class="search-form">
            <input type="text" name="search" placeholder="Search products..." value="` + searchQuery + `" class="search-input">
	...
	if searchQuery != "" {
		if len(products) == 0 {
			...
			html += `<div class="search-results">
                <p>Found <strong>` + strconv.Itoa(len(products)) + `</strong> product(s) matching "<strong>` + searchQuery + `</strong>"</p>
            </div>`
		}
	}
...
func cartHandler(w http.ResponseWriter, r *http.Request) {
	...
	// SECURE: Authentication required for cart access
	if !isLoggedIn {
...
<head>
    ...
    <style>
        ...
        .security-notice { background: #d4edda; border: 1px solid #c3e6cb; color: #155724; padding: 1rem; border-radius: 4px; margin-bottom: 2rem; }
    </style>
</head>
<body>
    <div class="nav">
    ...
    <div class="container">
        <div class="security-notice">
            ...
            <p>You are viewing your personal cart. All data is isolated to your account only.</p>
        </div>
	...
	if len(cart) == 0 {
		...
		for _, item := range cart {
            ...
            </div>`, item.Name, item.Price, item.Quantity, item.Price*float64(item.Quantity), 0)
		}
...
func checkoutHandler(w http.ResponseWriter, r *http.Request) {
	session, _ := store.Get(r, "session")
	userID, isLoggedIn := session.Values["user_id"].(int)

	// SECURE: Authentication required for checkout
	if !isLoggedIn {
		http.Redirect(w, r, "/login?error=authentication_required", http.StatusSeeOther)
		return
	}
...
<body>
    ...
    <div class="container">
        ...
        <div class="security-notice" style="background: #d4edda; border: 1px solid #c3e6cb; color: #155724; padding: 1rem; border-radius: 4px; margin: 1rem 0;">
            <h3>🔒 Secure Checkout</h3>
            <p>You are completing a secure purchase with your authenticated account. All data is protected and isolated to your account only.</p>
        </div>
...
func ordersHandler(w http.ResponseWriter, r *http.Request) {
	...
	// SECURE: Authentication required for orders access
	if !isLoggedIn {
		http.Redirect(w, r, "/login?error=authentication_required", http.StatusSeeOther)
		return
	}

	// SECURE: Customer isolation - only show user's own orders
	rows, err := db.Query(`
...
<body>
    ...
    <div class="container">
        <div class="security-notice" style="background: #d4edda; border: 1px solid #c3e6cb; color: #155724; padding: 1rem; border-radius: 4px; margin-bottom: 2rem;">
            <h3>🔒 Secure Customer Access</h3>
            <p>You are viewing your personal orders. All data is isolated to your account only.</p>
        </div>
        <h2>Your Orders</h2>`
	...
	if len(orders) == 0 {
		...
		for _, order := range orders {
            ...
            </div>`, order.ID, order.Total, order.CreatedAt.Format("January 2, 2006 at 3:04 PM"), order.ID)
		}
	}
...
func orderDetailHandler(w http.ResponseWriter, r *http.Request) {
	...
	// SECURE: Authentication required for order details access
	if !isLoggedIn {
		http.Redirect(w, r, "/login?error=authentication_required", http.StatusSeeOther)
		return
	}
	...
	// SECURE: Customer isolation - only allow access to user's own orders
	var order Order
	err := db.QueryRow(`
		SELECT o.id, o.user_id, o.total, o.created_at, u.email 
		FROM orders o 
		JOIN users u ON o.user_id = u.id 
		WHERE o.id = ? AND o.user_id = ?
	`, orderID, userID).Scan(&order.ID, &order.UserID, &order.Total, &order.CreatedAt, &order.UserEmail)
	...
	templates.ExecuteTemplate(w, "order_detail.html", order)
}
...
func paymentsHandler(w http.ResponseWriter, r *http.Request) {
	ownerMiddleware(func(w http.ResponseWriter, r *http.Request) {
		// Get all payment information (for educational purposes - this is intentionally insecure!)
		rows, err := db.Query(`
			SELECT p.id, p.order_id, p.card_number, p.card_holder_name, 
				   p.expiration_month, p.expiration_year, p.cvv, p.payment_date,
				   o.total, u.email
			FROM payments p
		...
		for rows.Next() {
			...
			err := rows.Scan(&payment.ID, &payment.OrderID, &payment.CardNumber, &payment.CardHolderName,
				&payment.ExpirationMonth, &payment.ExpirationYear, &payment.CVV, &payment.PaymentDate,
		...
		templates.ExecuteTemplate(w, "payments.html", map[string]interface{}{
			"Payments": payments,
		})
	})(w, r)
}
